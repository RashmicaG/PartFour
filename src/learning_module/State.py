...
class State:
    ...
    def createStateActions(self):
        actionable_blocks = copy.deepcopy(self.configuration)        
        ...
        for nb in non_actionable_blocks:
            actionable_blocks[nb] = -2
        for ablock, ablock_status in enumerate(actionable_blocks):
            if(ablock_status!= -2):
                if(ablock_status >-1):
                    actions.append(StateAction(ablock))
                for dblock, dblock_status in enumerate(actionable_blocks):
